...
import {ProductFilterFormData, ProductFilterParams} from "@/types/product.type";
...
export default function ProductFilters({productTypes, onFilters, initialFilters}: Readonly<ProductFilterProps>) {
    ...
    return (
        <Form {...form}>
            <form id="product-filters" onSubmit={form.handleSubmit(onSubmitFilters)} className="h-full flex flex-col justify-between gap-2">
                <div className="mb-4">
                    <FormField
                        name="content"
                        render={({field}) => (
                ...
                <div className="flex flex-row gap-2 w-full">
                    <Button size="sm" variant="ringHover" className="w-full" onClick={() => form.reset(defaultFormValues)}>
                        <IconRefresh size={16} className="mr-2"/>
                        {tCommon("btnReset")}
                    </Button>
                    <Button size="sm" variant="ringHover" className="w-full" form="product-filters" type="submit">
