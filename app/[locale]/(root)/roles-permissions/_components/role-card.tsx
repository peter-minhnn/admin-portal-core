...
const RoleCard = memo(({ role, className, roles }: RoleCardProps) => {
    ...
    return (
      <div className="mt-4 flex items-center gap-2 min-h-[32px]">
        <div className="flex items-center">
          ...
          {role.users?.length > 4 && (
            <span className="flex items-center justify-center z-10 h-8 w-8 -ml-1.5 text-xs font-medium text-white bg-gray-500 border-2 border-white rounded-full hover:bg-gray-600 dark:border-gray-800">
              +{role.users?.length - 4}
            </span>
          )}
        </div>
        <span>{t('totalUsers', { userCount: role.users?.length ?? 0 })}</span>
      </div>
    );
  }, [role, t]);
    ...
    <div className={cn('rounded-lg border border-solid p-6', className)}>
      <header className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          ...
          <h4 className="font-medium">{role.roleName?.toLocaleUpperCase()}</h4>
        </div>
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56" align="end" forceMount>
            <DropdownMenuGroup>
              <DropdownMenuItem>{t('addUser')}</DropdownMenuItem>
              <DropdownMenuItem>{t('renameRole')}</DropdownMenuItem>
              <DropdownMenuItem>{t('deleteRole')}</DropdownMenuItem>
            </DropdownMenuGroup>
          </DropdownMenuContent>
        </DropdownMenu>
      </header>
      ...
      <Button
        ...
        onClick={() => {
          openModal({
            isOpen: true,
            title: t('userManagementTitleModal'),
            description: t('userManagementDescriptionModal'),
            modalContent: (
              <UserRoleManagement
                role={role}
                roles={roles.filter((v) => v.roleCode !== role.roleCode)}
              />
            ),
